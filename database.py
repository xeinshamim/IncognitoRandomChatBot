...
def ensure_connections(func):
    """ Decorator for connection to the DBMS: opens the connection, executes the passed function and closes the connection after itself.
         Secure connection!
        """
...
def init_db(conn, force: bool = False):
    """ Check if the table exists, if not recreate it

           :param conn: connect to the DBMS
           :param force: explicitly recreates all tables
       """
    c = conn.cursor()
    if force:
...
@ensure_connections
def get_info(conn, user_id: int):  # Retrieving all user information from the users table
    c = conn.cursor()
...
@ensure_connections
def add_user(conn, first_id: int):  # Add the first user to the queue
    c = conn.cursor()
...
@ensure_connections
def select_free(conn):  # Search for a user who does not have a partner in the queue
    c = conn.cursor()
...
@ensure_connections
def add_second_user(conn, first_id: int, second_id: int):  # Add a second user to the queue
    c = conn.cursor()
...
def check_status(conn, first_id: int, second_id: int):
  # Checking if it is possible to connect these two users in 
    # a queue, if they meet all the parameters
...
def check_change(conn, first_id: int,
                 second_id: int):  # Check for compatibility of search type and gender of two users
    c = conn.cursor()
    first_change = False
    ...
    if not change == "All":
        if change == "Male":
            c.execute('SELECT EXISTS(SELECT * FROM users WHERE user_id=? AND gender = "Male")', (second_id,))
            ...
            c.execute('SELECT EXISTS(SELECT * FROM users WHERE user_id=? AND gender = "Female")', (second_id,))
            if c.fetchone()[0]:
                first_change = True
            else:
                return False
    ...
    if not change == "All":
        if change == "Male":
            c.execute('SELECT EXISTS(SELECT * FROM users WHERE user_id=? AND gender = "Male")', (first_id,))
            ...
            c.execute('SELECT EXISTS(SELECT * FROM users WHERE user_id=? AND gender = "Female")', (first_id,))
            if c.fetchone()[0]:
                second_change = True
            else:
                return False
...
@ensure_connections
def check_companion(conn, first_id: int):  # Getting the id of the user with whom he is associated from the queue
    c = conn.cursor()
    c.execute(
...
@ensure_connections
def close_chat(conn, first_id: int):  # Changes the status to closed, which means their communication has stopped
    c = conn.cursor()
